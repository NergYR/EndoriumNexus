...
export function AuditPage() {
  ...
  if (audit.isLoading || !audit.data) {
    return <div className="text-sm text-slate-400">Loading audit log...</div>;
  }
    ...
    <Panel title="Audit Log" eyebrow="Traceability">
      <div className="space-y-3">
        {audit.data.map((event) => (
          <article className="rounded-2xl border border-white/8 bg-white/4 px-4 py-4" key={event.happenedAt + event.action}>
