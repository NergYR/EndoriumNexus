...
export function Wizard({
  ...
  return (
    <div className="rounded-3xl border border-white/8 bg-black/15">
      <div className="border-b border-white/8 px-4 py-4">
        <p className="text-xs uppercase tracking-[0.24em] text-cyan-200/60">Guided wizard</p>
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[220px_1fr]">
        ...
        <div className="min-w-0">
          {children}
          {error ? <p className="mt-3 rounded-2xl border border-rose-300/20 bg-rose-300/10 px-4 py-3 text-sm text-rose-100">{error}</p> : null}
...
export function WizardSummary({ items }: { items: Array<{ label: string; value: ReactNode }> }) {
  return (
    <dl className="grid gap-2 rounded-2xl border border-white/8 bg-slate-950/70 p-4 text-sm">
      {items.map((item) => (
        <div className="grid gap-1 border-b border-white/6 pb-2 last:border-b-0 last:pb-0" key={item.label}>
          <dt className="text-xs uppercase tracking-[0.16em] text-slate-500">{item.label}</dt>
          <dd className="break-all text-slate-100">{item.value}</dd>
        </div>
      ))}
    </dl>
  );
